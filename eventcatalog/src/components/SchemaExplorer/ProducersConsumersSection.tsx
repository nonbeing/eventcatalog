    ...
    <div className="flex-shrink-0 border-b border-gray-200 dark:border-gray-700">
      ...
      >
        <div className="flex items-center gap-2">
          ...
          <span className="text-xs font-semibold text-gray-900 dark:text-gray-100">Producers & Consumers</span>
      ...
      {isExpanded && (
        <div className="px-4 pb-2 bg-gray-50 dark:bg-gray-800/50">
          {producers.length > 0 && (
            <div className="mb-3">
              <h4 className="text-xs font-semibold text-gray-900 dark:text-gray-100 mb-2">Producers ({producers.length})</h4>
              <div className="flex flex-wrap gap-2">
                {producers.map((producer: Producer, idx: number) => (
                  <a
                    key={`${producer.id}-${idx}`}
                    href={buildUrl(`/docs/services/${producer.id}/${producer.version}`)}
                    className="inline-flex items-center gap-1.5 pl-1 pr-3 py-1 text-xs font-medium text-gray-700 dark:text-gray-300 bg-white dark:bg-gray-700 border border-gray-200 dark:border-gray-600 rounded-full hover:border-gray-300 dark:hover:border-gray-500 hover:shadow-sm transition-all"
                  ...
                  >
                    ...
                    <span className="text-gray-500 dark:text-gray-400 text-[11px]">v{producer.version}</span>
                  </a>
                ))}
              </div>
            </div>
          ...
          {consumers.length > 0 && (
            <div>
              <h4 className="text-xs font-semibold text-gray-900 dark:text-gray-100 mb-2">Consumers ({consumers.length})</h4>
              <div className="flex flex-wrap gap-2">
                {consumers.map((consumer: Consumer, idx: number) => (
                  ...
                  >
                    ...
                    <span className="text-gray-500 dark:text-gray-400 text-[11px]">v{consumer.version}</span>
                  </a>
                ))}
              </div>
            </div>
