...
const Message = memo(({ message, collection }: { message: any; collection: string }) => {
  ...
  return (
    <a
      ...
      className="group flex border border-gray-200 dark:border-gray-600 items-center gap-1 rounded-md text-[11px] font-medium hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors duration-200 bg-white dark:bg-gray-800"
    >
      <div className="bg-white dark:bg-gray-700 border-r border-gray-200 dark:border-gray-600 px-2 py-1.5 rounded-l-md">
        <Icon className={`h-3 w-3 text-${color}-500`} />
      </div>
      <span className="px-1 py-1 truncate max-w-[140px] text-gray-900 dark:text-gray-100">{message.data.name}</span>
    </a>
  );
});
...
const MessagesContainer = memo(
  ({ messages, type, selectedTypes }: { messages: any[]; type: 'receives' | 'sends'; selectedTypes: string[] }) => {
    ...
    return (
      <div className={`flex-1 h-full flex flex-col bg-${bgColor}-100 dark:bg-${bgColor}-900/30 border border-${bgColor}-300 dark:border-${bgColor}-700 rounded-lg p-4`}>
        <div className="space-y-2 flex-1">
          ...
          {remainingMessagesCount > 0 && (
            <div className="text-center py-1">
              <p className="text-gray-500 dark:text-gray-400 text-[10px]">+ {remainingMessagesCount} more</p>
            </div>
          )}
          {(!messages?.length ||
...
const ServiceCard = memo(({ service, urlParams, selectedTypes }: { service: any; urlParams: any; selectedTypes: string[] }) => {
  return (
    <a
      ...
      className="group hover:bg-pink-50 dark:hover:bg-pink-900/20 bg-white dark:bg-gray-800 border-2 border-dashed border-pink-400 dark:border-pink-500 rounded-lg shadow-sm hover:shadow-lg transition-all duration-200 overflow-hidden"
    >
      <div className="p-6">
        <div className="flex items-center justify-between mb-3">
          <div className="flex items-center gap-2 w-full">
            <ServerIcon className="h-5 w-5 text-pink-500" />
            <h3 className="text-lg font-semibold text-gray-900 dark:text-gray-100 truncate group-hover:underline transition-colors duration-200 w-full max-w-[90%]">
              {service.data.name || service.data.id} (v{service.data.version})
            </h3>
          </div>
        </div>

        {service.data.summary && <p className="text-gray-600 dark:text-gray-400 text-sm line-clamp-2 min-h-[2.5rem]">{service.data.summary}</p>}
        ...
        {!urlParams?.serviceName && (
          <div className="flex items-center gap-4 mt-4">
            ...
            <div className="flex items-center gap-2 max-w-[200px]">
              ...
              <div className="bg-white dark:bg-gray-800 border-2 border-pink-100 dark:border-pink-800 rounded-lg p-4 shadow-sm">
                <div className="flex flex-col items-center gap-3">
                  <ServerIcon className="h-8 w-8 text-pink-500" />
                  <div className="text-center">
                    <p className="text-sm font-medium text-gray-900 dark:text-gray-100">{service.data.name || service.data.id}</p>
                    <p className="text-xs text-gray-500 dark:text-gray-400">v{service.data.version}</p>
                  </div>
                </div>
              </div>
                ...
                <div className="flex flex-wrap gap-1">
                  {service.data.readsFrom.slice(0, 3).map((container: any) => (
                    ...
                    >
                      ...
                      <span className="text-orange-800 dark:text-orange-300">{container.data.name}</span>
                    </a>
                  ))}
                ...
                <div className="flex flex-wrap gap-1">
                  {service.data.writesTo.slice(0, 3).map((container: any) => (
                    ...
                    >
                      ...
                      <span className="text-purple-800 dark:text-purple-300">{container.data.name}</span>
                    </a>
                  ))}
    ...
    return (
      <div className="space-y-6">
        ...
        {subdomains.map((subdomainRef: any) => {
          ...
          return (
            <div key={subdomain.data.id} className="bg-orange-50 dark:bg-orange-900/20 border-2 border-orange-400 dark:border-orange-600 rounded-lg p-6 space-y-4">
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-2">
                  <RectangleGroupIcon className="h-5 w-5 text-orange-500" />
                  <h3 className="text-xl font-semibold text-gray-900 dark:text-gray-100">{subdomain.data.name} (Subdomain)</h3>
                </div>
                <div className="flex gap-2">
                  <a
                    href={buildUrl(`/visualiser/domains/${subdomain.data.id}`)}
                    className="inline-flex items-center px-3 py-2 text-sm font-medium bg-white dark:bg-gray-800 text-gray-900 dark:text-gray-100 border border-gray-300 dark:border-gray-600 rounded-md transition-colors duration-200 hover:bg-gray-50 dark:hover:bg-gray-700"
                  >
                    View in visualizer
                  </a>
                  <a
                    href={buildUrl(`/docs/domains/${subdomain.data.id}`)}
                    className="inline-flex items-center px-3 py-2 text-sm font-medium text-gray-900 dark:text-gray-100 border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-800 rounded-md transition-colors duration-200 hover:bg-gray-50 dark:hover:bg-gray-700"
                  >
                    Read documentation
                  </a>
                </div>
              ...
              {subdomain.data.entities && subdomain.data.entities.length > 0 && (
                <div className="space-y-2">
                  <div className="flex items-center gap-2">
                    ...
                    <h4 className="text-xs font-semibold text-gray-700 dark:text-gray-300">Entities</h4>
                  </div>
                  <div className="flex flex-wrap gap-1">
                    {subdomain.data.entities.map((entity: any) => (
                      <a
                        ...
                        className="group inline-flex items-center gap-1 px-2 py-1 bg-purple-100 dark:bg-purple-900/30 border border-purple-300 dark:border-purple-700 rounded-md text-[11px] font-medium hover:bg-purple-200 dark:hover:bg-purple-900/50 transition-colors duration-200"
                      >
                        <BoxIcon className="h-3 w-3 text-purple-600 dark:text-purple-400" />
                        <span className="text-purple-800 dark:text-purple-300">{entity.id}</span>
                      </a>
                    ))}
                  </div>
                </div>
  ...
  return (
    <div>
      ...
      <nav className="mb-4 flex items-center space-x-2 text-sm text-gray-500 dark:text-gray-400">
        <a href={buildUrl('/architecture/domains')} className="hover:text-gray-700 dark:hover:text-gray-300 hover:underline flex items-center gap-2">
          <RectangleGroupIcon className="h-4 w-4" />
          Domains
        </a>
        <ChevronRightIcon className="h-4 w-4" />
        <a href={buildUrl('/architecture/services')} className="hover:text-gray-700 dark:hover:text-gray-300 hover:underline flex items-center gap-2">
        ...
        {urlParams?.domainId && (
          <>
            <ChevronRightIcon className="h-4 w-4" />
            <span className="text-gray-900 dark:text-gray-100">{urlParams.domainId}</span>
          </>
        )}
      </nav>
      ...
      {filteredAndSortedServices.length > 0 && (
        <div className={`rounded-xl overflow-hidden ${urlParams?.domainId ? 'bg-yellow-50 dark:bg-yellow-900/20 p-8 border-2 border-yellow-400 dark:border-yellow-600' : ''}`}>
